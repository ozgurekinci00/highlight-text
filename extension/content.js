...
const highlightSentences = async () => {
  ...
  if (mainContent) {
    ...
    console.log('KEY SENTENCES', keySentences);

    const keySentencesNormalized = keySentences.map(sentence => normalizeHyphens(sentence).toLowerCase());
    console.log('NORMALIZED', keySentencesNormalized);
    ...
    const highlightSentencesInNode = (node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        sentences.forEach(sentence => {
          const foundKeySentence = keySentencesNormalized.find(keySentence => {
            const normalizedSentence = normalizeHyphens(sentence).toLowerCase();
            const wordsInSentence = normalizedSentence.split(/\s+/);
            const wordsInKeySentence = keySentence.split(/\s+/);
    
            const firstFiveWordsInSentence = wordsInSentence.slice(0, 5).join(' ');
            const firstFiveWordsInKeySentence = wordsInKeySentence.slice(0, 5).join(' ');
